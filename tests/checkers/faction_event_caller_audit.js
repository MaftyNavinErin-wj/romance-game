...
function main() {
  ...
  // F1 修法(codex review):actual >= expected_count 时降为 INFO("已闭合"),不再 HIGH
  for (const [t, gap] of Object.entries(KNOWN_GAPS)) {
    ...
    if (missing > 0) {
      ...
      findings.push({
        kind: 'known_gap_closed',
        severity: 'INFO',
        msg: `eventType '${t}' 已知缺漏(${gap.d_class})已闭合:期望 ${gap.expected_count} 处 caller,实际 ${actual} 处达标`,
        candidate_d: gap.d_class,
      });
    }
  }
  ...
  lines.push('');
  lines.push('> Generated by `tests/checkers/faction_event_caller_audit.js`(无 timestamp,git commit 即可追溯)');
  lines.push('> 数据源:`project_romance_v181.html` + `src/**/*.js`');
  lines.push('> 服务 D 类:D-048 / D-049 / D-131(v130 重构推广不彻底模式)');
  lines.push('> **Sprint gate 能力**:✅ Yes — KNOWN_GAPS 内的 eventType 修复后 finding 自动从 HIGH 降为 INFO("已闭合"),可作为 batch 通过证据');
  lines.push('');
  lines.push('## 总览');
  lines.push('');
  lines.push(`| eventType | caller 数 | 状态 |`);
  lines.push('|---|---|---|');
  // F6 修法(codex review):状态文案精确化,避免"≥1 caller=完整覆盖"误导
  // - 0 caller → ❌
  // - KNOWN_GAPS 中且 actual<expected → ⚠️ 已知缺漏(N/M)
  // - KNOWN_GAPS 中且 actual≥expected → ✅ 已知缺漏闭合
  // - 不在 KNOWN_GAPS:present(≥1 caller,完整性未在 walkthrough 中校准)
  for (const t of KNOWN_EVENT_TYPES) {
    const n = byType[t].length;
    const known = KNOWN_GAPS[t];
    let status;
    if (n === 0) status = '❌ 0 caller';
    else if (known && n < known.expected_count) status = `⚠️ 已知缺漏 (${n}/${known.expected_count})`;
    else if (known) status = `✅ 已知缺漏闭合 (${n}/${known.expected_count})`;
    else status = `present (${n}, 完整性未校准)`;

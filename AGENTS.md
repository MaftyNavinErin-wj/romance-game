# AGENTS.md — Project Romance Refactor Constitution

> 这份文件是 Codex 启动每个 session 时**必须先读**的宪法。
...
1. **新功能 / 新设计先讨论 → 制作人 approve → 实装**。Codex 不擅自做设计决策。
...
1. 当前任务完成 + 必要验证完成 + commit pushed
2. context 余量预警(建议低于 20% 时收尾)
3. 制作人主动结束
4. 必要验证 FAIL 且 30 分钟内无法定位
5. 遇到设计决策需要回 Codex.ai 对话讨论
...
## Smoke Test 使用边界(仅重构行为保持)

`node tests/smoke.js` + `node tests/compare.js` 是**重构期行为保持测试**。
它的原始目的:在 data/render/chains/core 搬运、init/tick/save-load/AI/cross-chain 等高风险行为改动时,确认 refactor 前后 baseline 一致。

后续普通开发中,无论是 map design、文档、art workflow、UI/gameplay iteration、轻量数据调整,都**不默认用 smoke/compare 约束 commit**。
这些场景应使用更贴合变更的验证方式:人工 review、manifest/worklog 记录、截图/浏览器检查、语法检查、专项脚本、scenario validator、独立 code review 等。

只有以下场景才把 smoke/compare 作为 commit gate:

- 明确的重构搬运 / 行为保持任务
- init / tick / save-load / AI / cross-chain 机制改动
- 制作人明确要求本 session 跑 baseline compare
- 需要证明“行为 byte-identical / baseline-identical”的专项回归

如果 smoke/compare 在低风险或 docs-only session 中失败,只作为历史 baseline/环境信号报告,**不阻塞 commit**。
...
- 重构工作分支 → smoke PASS → squash merge 阶段分支
- 重构阶段分支 → 全量 smoke PASS + summary doc → merge main
- 非重构开发 / docs / map design → 使用本 session 适配的验证方式,不默认要求 smoke PASS

---

## 设计决策回流机制

如果 session 中遇到以下情况,**立即停止 + 回到 Codex.ai 对话讨论**:

1. 抽数据时发现循环依赖
2. 函数职责边界模糊,不知道该归到哪个模块
3. 接口设计需要选择(如 render 层是否能直读 G state)
4. 必要验证 FAIL 30 分钟无法定位
...
❌ "这里有个 bug 我顺便修了"
❌ "我觉得这样设计更好,我先实现看看"
❌ "重构行为保持任务 smoke FAIL,先 commit 再说"
❌ "重写一下 HANDOVER 让它更清晰"
❌ "加个新 feature 让用户体验更好"
❌ "这个 D 类很容易修,顺便 close 了吧"
❌ "高风险机制/重构改动不需要任何验证,代码看起来没问题"
...
✅ "重构 smoke FAIL,我先看 diff 报告"
✅ "这是个设计决策,我回 Codex.ai 讨论"
✅ "我发现一个新 D 类,记录到 followup.md,不当场 fix"
✅ "context 余量 25%,建议本 session 收尾"
✅ "搬完这个重构文件,smoke PASS,准备 commit"
✅ "这是 docs/map design 记录,不跑 refactor smoke,用人工 review + manifest/worklog 验证后 commit"

---

## 联系制作人(回流触发关键词)

如果 Codex 输出以下任一关键词,制作人应介入:

- 🛑 "BLOCKED: 需要设计决策"
- 🛑 "BLOCKED: 必要验证 FAIL 无法定位"
- 🛑 "BLOCKED: 发现循环依赖"
...
(AGENTS.md v1.0 完结 — 所有 session 启动必读)

...
- [Refactor phase status](project_refactor_status.md) — GEN_BASE audit 闭环 ✅ + **phase 6 wire W1-W3 ✅ push'd + W4a+W4b+W4c ✅ local** (`86843a3`/`49671ad`/`af83fe8`/`ceb5ff7`); **下次 W5 (在野/待出场池, 必带回归测试); W4a-c 整组完成 → 现在可实机测**。lesson: W-wire 必须全 G dump 实测 slice vs legacy verbatim;hidden state 坑 (isCapital key 集 / x/y / GENS_FULL 有 8 个 minTurn);W4+ byte-identical 故意破→换网 50 回合不崩+0 NaN+审差异;SCENARIO_214.generals[].post 是招牌头衔不是游戏官职;**官职有 stage 门槛 (STAGE_TIER1_SLOTS 只 regime 开一档大将军/丞相 / POST_TIERS 城数×stage 卡二三档),派官前必核 getPostSlots**;codex P2 lesson: 剧本可把 legacy-wild 武将提 active → active loop + wild loop 用 Set guard 防覆盖;**W4c: meta 走适配器模式 (getGenMeta 改读 m.genMeta 混合 composite — title/post/loyalty/relations←sc.generals + birthplace/clan/values/skills←GEN_BASE + gentry←legacy 透传, consumer 零改动);gentry/GEN_TAGS 类稀疏 legacy 数据无 GEN_BASE 家留 const,190 缺口记 followup;codex 标 pre-existing 缺陷为 "regression" 会误判,要用全 G dump byte-identical 反证**

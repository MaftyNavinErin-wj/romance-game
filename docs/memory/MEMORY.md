...
- [Refactor phase status](project_refactor_status.md) — GEN_BASE audit 闭环 ✅ + **phase 6 wire W1-W3 ✅ push'd + W4a step-1+2 + W4b ✅ local** (`86843a3`/`49671ad`/`af83fe8`); 下次 W4c (关系+亲密度+meta+小传+忠诚, 顺带解 W4b 遗留)。lesson: W-wire 必须全 G dump 实测 slice vs legacy verbatim;hidden state 坑 (isCapital key 集 / x/y / GENS_FULL 有 8 个 minTurn);W4+ byte-identical 故意破→换网 50 回合不崩+0 NaN+审差异;SCENARIO_214.generals[].post 是招牌头衔不是游戏官职;**官职有 stage 门槛 (STAGE_TIER1_SLOTS 只 regime 开一档大将军/丞相 / POST_TIERS 城数×stage 卡二三档),派官前必核 getPostSlots**;codex P2 lesson: 剧本可把 legacy-wild 武将提 active → active loop + wild loop 用 Set guard 防覆盖

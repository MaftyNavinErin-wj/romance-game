...
OUT = ROOT / "terrain_t06_cc_w_no_city_v2_city_stamp_composite_v1.png"
PREVIEW = ROOT / "terrain_t06_cc_w_no_city_v2_city_stamp_composite_v1_preview.png"
...
def paste_stamp(canvas, stamp, center, cls):
...
def draw_review_overlay(img, placements):
    ...
    draw.text(
        (38, 32),
        "T06 v2 city stamp cutout composite v1 - review labels only, no runtime/map promotion",
...
def main():
    ...
    for city_id in [
        ...
        bbox = paste_stamp(canvas, stamps[source_key(cls)], p, cls)
        placements.append(
            {
                "id": city_id,
                "name": city["name"],
    ...
    manifest_json = ROOT / "terrain_t06_cc_w_no_city_v2_city_stamp_composite_v1_placements.json"

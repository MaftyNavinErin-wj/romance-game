...
//   - 数据 / 常量(部分已抽 src/data/):FAC / CITIES_DEF / DIPLO_INIT
//     / FAC_IDENTITY
//     / RETAINER_LEVEL / INTIMACY_PRESET / ETHOS_INIT / GEN_TAGS
//     / getScenarioFactions() / ALL_POSTS / WILD_GENS / CITY_MAP
//     / STATE_TO_GENTRY_FAC
...
//     §8.4 W6-pending-3: GENS_FULL 已退役 → m.GENS_FULL (W4a 真值) + getGenOrigFac helper
...
function initGame(scenarioId){
  ...
  _allGensForInit.forEach(g=>{
    ...
    // F-W4c-2 part 2 (制作人 decision): 旧 hardcoded 214 fid map → ruler 全名
    //   (设计意图: 「仕于X」X 是君主名, 不是地域/族号; 修 190 显示 "仕于caocao" + 214 改 "仕于曹操" 等)
    //   故意破 byte-identical, 重拍 baseline。
    const facName=fid ? (getFactionDef(fid)?.ruler || fid) : '在野';

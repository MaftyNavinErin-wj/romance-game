...
function showBreakdown(e, type, cityId){
  ...
  if(type==='food'){
    ...
    if(city.prefect){
      ...
      const _bdPrefDeployed = G.units.some(u=>(u.status==='march'||u.status==='siege') && u.squads.some(sq=>sq.genName===city.prefect));
      const _bdPrefMult = 1 + (_bdPrefPol / 500) * (_bdPrefDeployed ? 0.5 : 1.0);
      if(_bdPrefMult > 1.001) html += `<div class="bd-row"><span class="bd-label">太守政治（${city.prefect}·政${_bdPrefPol}${_bdPrefDeployed?'·出征减半':''}）</span><span class="bd-val pos">×${_bdPrefMult.toFixed(3)}</span></div>`;
    }
    // ★ v136: 官职/朝议金产buff
    ...
    if(city.prefect){
      ...
      const _deployed = G.units.some(u=>(u.status==='march'||u.status==='siege')&&u.squads.some(sq=>sq.genName===city.prefect));
      const _half = _deployed ? 0.5 : 1.0;
      prefectD = (_polM / 400) * _half;
      prefectLabel = `${city.prefect}（政${_polM}${_deployed?'·出征减半':''}）`;
    } else {
